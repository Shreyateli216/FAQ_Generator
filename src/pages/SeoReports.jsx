        ...
        <div className="glass-card p-6 flex flex-col items-center justify-center text-center col-span-1">
           <div className="relative w-32 h-32 flex items-center justify-center mb-4">
              <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 100 100">
                 ...
                 <circle cx="50" cy="50" r="45" fill="none" stroke="#00F0FF" strokeWidth="8" strokeDasharray={`${MOCK_SEO_DATA.score * 2.82} 282`} className="transition-all duration-1000 ease-out" />
              </svg>
              <div className="absolute flex flex-col items-center">
                 <span className="text-3xl font-bold text-white">{MOCK_SEO_DATA.score}</span>
                 <span className="text-[10px] uppercase text-white/50 tracking-wider">Overall</span>
              </div>
           </div>
           <h3 className="font-semibold text-white/90">Excellent Score</h3>
           <p className="text-xs text-white/50 mt-1">Ready for production deployment.</p>
        </div>

        <div className="glass-card p-6 col-span-1 md:col-span-3 grid grid-cols-2 gap-4">
           {Object.entries(MOCK_SEO_DATA.metrics).map(([key, val]) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
        <div className="glass-card p-6 flex flex-col">
          ...
          <div className="space-y-3 flex-1 overflow-y-auto pr-2">
            {MOCK_SEO_DATA.suggestions.map((sug, i) => (
